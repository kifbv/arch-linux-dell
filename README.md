How to install Arch Linux on the Dell XPS13 (9360).

The instructions below are mostly from the Arch Linux [Installation Guide](https://wiki.archlinux.org/index.php/Installation_guide/).
...
## Pre-installation
Note: if you have some SQUASHFS errors during installation, it probably means that your USB device is not good so try another one.

### Set the console keyboard layout
The default is `us` but for e.g. a `uk` layout:
1. list the available layouts: 
1. `localectl list-keymaps` or
1. `ls /usr/share/kbd/keymaps/**/*.map.gz`
2. load the uk layout: `loadkeys uk`
...
### Partition the disk
Check what disk you have with `lsblk` or `fdisk -l` and take a note of the device name (`/dev/nvme0n1` on the Dell XPS13)
...
I don't give a label to the partitions because Luks encryption doesn't support it and it's the UUID that will identify the disk.

### Encrypt the Linux partition
I choose to use [LVM on LUKS](https://wiki.archlinux.org/index.php/Dm-crypt/Encrypting_an_entire_system#LVM_on_LUKS).

1. Create the encrypted container on the **system** partition (choose a good passphrase):
`cryptsetup luksFormat /dev/nvme0n1p2`
1. Open the container (I name it 'luks' you, can use something else):
`cryptsetup open /dev/nvme0n1p2 luks`
1. Create a Physical Volume on top of the opened LUKS container:
`pvcreate /dev/mapper/luks`
1. Create a Volume Group and add the Physical Volume to it (I name it `main`):
...
We install the [base](https://www.archlinux.org/groups/x86_64/base/) and [base-devel](https://www.archlinux.org/groups/x86_64/base-devel/) package groups as well as some packages for wireless network configuration, Intel micro-code updates and git:
`pacstrap /mnt base base-devel dialog git intel-ucode dhcpcd wpa_actiond wpa_supplicant`

## Configure the system
### Fstab
Then we create the `fstab` file (verify it and use `lazytime` mount option<sup>see [fstab](#fstab)</sup>):
...
### Locale
Uncomment needed localisations in `/etc/locale.gen` and regenerate them with `locale-gen`.

Set the `LANG` variable `/etc/locale.conf` e.g. `LANG=en_GB.UTF-8`
...
### Hostname
Create the hostname file: `echo <hostname> > /etc/hostname` and add a matching entry to `/etc/hosts`: `echo 127.0.1.1 <hostname>.localdomain <hostname> >> /etc/hosts`

### Network configuration
Configure wireless with `wifi-menu`.
Configure wireless to start at boot and automatically connect to known networks: `systemctl enable netctl-auto@<interface_name>.service` (use tab completion to figure out the interface name).
...
I run `visudo` and add the following to have my user in the sudores file :
```Shell
# options
